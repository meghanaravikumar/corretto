...
import org.bdgenomics.corretto.{ Corretto, CorrettoFunSuite }
...
class ErrorCorrectionSuite extends CorrettoFunSuite {
  ...
  ignore("correct reads from a mouse") { //needs to be a sparkTest -- having issues calling Corretto
    ...
    Corretto(Array(readsFilepath, "./output_test.adam")).run(sc)

    val readsToLoad = sc.loadAlignments("./output_test.adam")

    assert(reads.count === readsToLoad.count)
  }
